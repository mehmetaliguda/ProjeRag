...
class RoomManager:
    ...
    def delete_room(self, room_id: str) -> None:
        """Odayi RAM'den, diskten (PDF + chroma_db + images) ve DB'den kaldirir."""
        row = Room.query.get(room_id)
        if row is None and room_id not in self._rooms:
            raise FileNotFoundError(f"Oda bulunamadi: {room_id}")

        # Once RAM'den cikar; boylece Chroma/embedding handle'lari birakilir
        # ve ardindan klasoru silmek Windows/posix'te dosya kilidi sorunu yaratmaz.
...
def get_multi_room_answer(room_ids: List[str], mesaj: str) -> dict:
    ...
    for room in rooms:
        ...
        # optimized_query her room icin ayni metin oldugundan, embed_query
        # cagrisi _CachedEmbeddings sayesinde pratikte sadece ilk room'da
        # gercek bir Ollama istegi yapar, digerleri cache'ten doner.
    ...
    reranked = _multi_room_reranker.rerank(mesaj, all_results, min_score=MULTI_ROOM_MIN_RERANK_SCORE)[:top_k]
