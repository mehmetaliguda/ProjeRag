...
#
# flask_deneme.py'nin yerini alir. Fark: artik tek PDF yerine N tane oda var,
# her istek hangi oda ile konusuldugunu 'room' alaniyla belirtir.
...
def list_rooms():
    """Homepage'in gosterecegi PDF/oda listesi."""
    return jsonify({"rooms": room_manager.list_rooms()})


@app.route('/rooms', methods=['POST'])
def upload_room():
    """Yeni PDF yukleme: multipart/form-data, alan adi 'file'."""
    ...
    try:
        # NOT: Buyuk PDF'lerde indexleme (embedding) suresi uzun olabilir,
        # bu cagri o sure boyunca bloklar. Ileride bunu bir arka plan
        # gorevine (thread/queue) tasimak isteyebilirsin.
